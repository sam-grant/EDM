...
void Run(string dataset) { 
  ...
  TString foutName = "../Plots/Data/dMu/Run-1a/Plots/verticalOffset_"+dataset+".root";
  ...
  for(auto& step : step_) {
    ...
    for(auto& stn : stn_) {
      ...
      TH2D *h_theta_y_vs_p = (TH2D*)fin->Get(("VerticalOffsetPlots/"+step+"MeV/"+stn+"_ThetaY_vs_Momentum").c_str());

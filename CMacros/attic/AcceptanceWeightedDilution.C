...
void OverlayAcceptanceFractions(vector<TGraphErrors*> gr_, string title, string fname) { 
...
void Run(bool write) {
	...
	vector<TGraphErrors*> gr_ratio_;

	for(auto& stn : stn_) {

		TString f3Name = "../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_allDecays_WORLD_250MeV_AQ_noVertCorr_accWeight"+stn+"_full.root";
		...
		// Draw graphs
		DrawOverlay(gr1_reset, gr2_reset, gr3_reset, stn+";Momentum [MeV];A_{EDM} [mrad] / 250 MeV", "../Images/MC/Acceptance/truth/FullCorrectionResults/"+stn+"_gr_AcceptanceCorrected_AEDM_vs_p_overlay");
		
		//DrawOverlay2(gr1_reset, gr3_reset, stn+";Momentum [MeV];A_{EDM} [mrad] / 250 MeV", "../Images/MC/Acceptance/truth/FullCorrectionResults/"+stn+"_AcceptanceCorrected_AEDM_vs_p_partial_overlay");

		// Calculate acceptance scale factors based on the ratio of weighted to unweighted decays
		...
		for (int i(0); i<gr1_reset->GetN(); i++) {
			...
			if(x1!=x3) cout<<"WARNING: x-values for weighted and unweighted graphs differ"<<endl;

			double y = gr3_reset->GetY()[i]/gr1_reset->GetY()[i];

			// gr3 is a subset of gr1, so take it's stat error

			double ey = gr3_reset->GetEY()[i];
		...
		gr_ratio_.push_back(gr_ratio);

		DrawTGraphErrors(gr_ratio, stn+";Decay vertex momentum [MeV];A_{EDM} acceptance factor / 250 MeV", "../Images/MC/Acceptance/truth/FullCorrectionResults/"+stn+"_gr_AEDM_acceptanceFactors");

	}

	OverlayAcceptanceFractions(gr_ratio_, ";Decay vertex momentum (station 12) [MeV];A_{EDM} acceptance factor / 250 MeV", "../Images/MC/Acceptance/truth/FullCorrectionResults/gr_AcceptanceScaleFactors_AEDM_vs_p_overlay");
	...
	// What I would prefer to do, pending an examination of 

/*	
		// Calculate acceptance factors

		// Confused now, didn't I modify this code?
		...
		for (int i(0); i<gr1_reset->GetN(); i++) {
			...
			// `the assumption is that these uncertainties aer uncorrelated.
		...
		TH1D *h1_ratio_alt = new TH1D((stn+"_ratio_alt").c_str(), (stn+";Decay vertex momentum [MeV];A_{EDM} acceptance factor / 250 MeV").c_str(), gr_ratio_alt->GetN(), xmin, xmax);

...
/*void DrawMainOverlay(TGraphErrors *gr0, TGraphErrors *gr1, TGraphErrors *gr2, std::string title, std::string fname, std::string suffix) {
	...
	// range
	double xmin = gr0->GetX()[0];
	double xmax = gr0->GetX()[gr0->GetN()-1];
	gr0->GetXaxis()->SetRangeUser(xmin - 100, xmax + 100);
	gr0->GetYaxis()->SetRangeUser(-0.05, 0.35);
	gr0->Draw("AP");

	gr1->SetMarkerStyle(20); // open circle
	gr1->SetMarkerColor(kRed);
	gr1->SetLineColor(kRed);
	gr1->Draw("P SAME");
	...
	if(gr2==0) {
		if(suffix=="1mm") {
			l->AddEntry(gr1, "Truth vertices (#plus1 mm)");
			l->AddEntry(gr2, "Truth vertices (#minus1 mm)");
		} else {
...
void DrawMainOverlay(TGraphErrors *gr1, TGraphErrors *gr2, TGraphErrors *gr3, std::string title, std::string fname, std::string suffix) {
...
/*	if(gr3!=0) {
	...
	if(gr3!=0) {
		...
		if(suffix=="plus1mm") {
			l->AddEntry(gr3, "Truth vertices (#plus1 mm)");
		} else if(suffix=="minus1mm") {
			l->AddEntry(gr3, "Truth vertices (#minus1 mm)");
		} else if(suffix=="plus0.1deg") {
			l->AddEntry(gr3, "Truth vertices (#plus0.1#circ)");
		} else if(suffix=="minus0.1deg") {
			l->AddEntry(gr3, "Truth vertices (#minus0.1#circ)");
		}
...
void DrawSecondaryOverlay(TGraphErrors *gr1, TGraphErrors *gr2, string stn, std::string title, std::string fname, string suffix) {
	...
	if(suffix=="plus1mm") {
		l->AddEntry(gr2, "Truth vertices (#plus1 mm)");
	} else if(suffix=="minus1mm") {
		l->AddEntry(gr2, "Truth vertices (#minus1 mm)");
	} else if(suffix=="plus0.1deg") {
		l->AddEntry(gr2, "Truth vertices (#plus0.1#circ)");
	} else if(suffix=="minus0.1deg") {
		l->AddEntry(gr2, "Truth vertices (#minus0.1#circ)");
	}
...
void Run(bool write, string suffix = "plus1mm") {

	TString foutName = "../Plots/MC/Acceptance/Plots/acceptanceFactorErrorVsMomentum_250MeV_"+suffix+".root";
	if(!write) foutName = "delete_me.root";

	TFile *fout = new TFile(foutName, "RECREATE");
	fout->mkdir("graphs"); fout->mkdir("hists");

	string f0Name = "../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_allDecays_WORLD_250MeV_AQ_noVertCorr_full.root"; 

	// Dilution curve
	string f1Name = "../Plots/MC/dMu/Dilution/dilutionCurves.exact.root";

	// Nominal file
	string f2Name = "../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_trackTruth_WORLD_250MeV_BQ_noVertCorr_full.root"; 
	string f3Name = "../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_trackTruth_WORLD_250MeV_BQ_noVertCorr_"+suffix+"_full.root";
	
	TFile *f0 = TFile::Open(f0Name.c_str());
	...
	// All decays
	TGraphErrors *gr0 = (TGraphErrors*)f0->Get("MomentumBinnedAnalysis/ParameterScans/AEDM_vs_p_thetaY");
	
	// Nominal
	TGraphErrors *gr1 = (TGraphErrors*)f2->Get(("MomentumBinnedAnalysis/ParameterScans/"+stn+"_AEDM_vs_p_thetaY").c_str());

	// Shifted
	TGraphErrors *gr2 = (TGraphErrors*)f3->Get(("MomentumBinnedAnalysis/ParameterScans/"+stn+"_AEDM_vs_p_thetaY").c_str());

	gr0 = ResetGraph(gr0, xmin, xmax);
	gr1 = ResetGraph(gr1, xmin, xmax);
	gr2 = ResetGraph(gr2, xmin, xmax);

	// Draw graphs
	DrawMainOverlay(gr0, gr1, gr2, ";Decay vertex momentum [MeV];A_{EDM} [mrad] / 250 MeV", "../Images/MC/Acceptance/truth/CorrectionResults/"+stn+"_AlignmentShifted_AEDM_vs_p_overlay_"+suffix, suffix);


	// Calculate the acceptance scale factors


	// Calculate the shift in acceptance scale factors?
	...
	// TODO: put these two loops in a loop
	...
	for (int i(0); i<gr1->GetN(); i++) {
		...
		double y = (gr1->GetY()[i]/delta_calc) * (1/dilutionFunc->Eval(x));
		double ey = y * (gr1->GetEY()[i]/gr1->GetY()[i]); 
	...
	// Make ratio of gr3/gr1 (gr3 = shift)
	...
	fout->cd("graphs");
	gr_ratio_1->SetName(("gr_ratio_plus"+suffix).c_str());
	gr_ratio_1->Write();

	gr_ratio_2->SetName(("gr_ratio_minus"+suffix).c_str());
	gr_ratio_2->Write();

	DrawSecondaryOverlay(gr_ratio_1, gr_ratio_2, "", ";Decay vertex momentum (nominal) [MeV];A_{EDM} acceptance fraction / 250 MeV", "../Images/MC/Acceptance/truth/CorrectionResults/OverlayAEDMAcceptanceFraction_"+suffix, suffix);
/*
	f1->Close();
	f2->Close();
	f3->Close();
...
/*		if(stn=="S12") {
		...
		double ey = 0;//sqrt(pow(gr_ratio_1->GetEY()[i]*n_0/N_0,2)+pow(gr_ratio_2->GetEY()[i]*n_1/N_1,2));
...
// Simplify, ignore nominal, just consider shifted samples



void AlignmentComparison2() {
	...
	Run(false, "plus1mm");
	//Run(false, "minus1mm");
	//Run(false, "plus0.1deg");
	//Run(false, "minus0.1deg");

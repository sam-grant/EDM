...
void DrawOverlayB(TGraphErrors *gr_weight, TGraphErrors *gr_tracks, TGraphErrors *gr_align, string align, string title, string fname) {
	...
	//gr_align->SetMarkerStyle(24); // open circle
	...
	TLegend *l = new TLegend(.49, .79, .89, .89);
	l->SetBorderSize(0);
	l->SetTextSize(24);
	l->SetTextFont(44);
...
void OverlayAlignDiffGraphs(TGraphErrors *gr1, TGraphErrors *gr2, string shift, std::string title, std::string fname) {
	...
	gr1->GetYaxis()->SetRangeUser(-.2, .2);//ymin-0.25, ymax+0.25);
	...
	TLegend *leg = new TLegend(0.15, 0.79, 0.39, 0.89);
	leg->SetBorderSize(0);
	leg->SetNColumns(2);
	leg->SetTextSize(24);
	leg->SetTextFont(44);
...
1. Get all files, including all four alignment files
2. Plot A_EDM
3. Calculate the acceptance scale factors for each 
4. Make difference plots 
5. Write to file in a sensible way
...
void Run(bool write, string dataset = "Run-1a") { 
	...
	//vector<TGraphErrors*> gr_ratio_misalign_;
	...
	for(auto& stn : stn_) {
		...
		for(auto& alignStr : alignStr_) { 
			...
			for (int i(0); i<gr_ratio_align->GetN(); i++) {
				...
				double ey = sqrt(pow(gr_ratio_main->GetEY()[i],2)+pow(gr_ratio_align->GetEY()[i],2));

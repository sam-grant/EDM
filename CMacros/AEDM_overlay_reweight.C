...
void Run(string stn) { 
	...
	TFile *f0 = TFile::Open(("../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_allDecays_WORLD_250MeV_AQ_noVertCorr_accWeight"+stn+"_full.root").c_str());
	TFile *f1 = TFile::Open(("../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_allDecays_WORLD_250MeV_AQ_noVertCorr_accWeight"+stn+"_full_reweight"+stn+"Run-1a.root").c_str());
	TFile *f2 = TFile::Open(("../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_allDecays_WORLD_250MeV_AQ_noVertCorr_accWeight"+stn+"_full_reweight"+stn+"Run-1b.root").c_str());
	TFile *f3 = TFile::Open(("../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_allDecays_WORLD_250MeV_AQ_noVertCorr_accWeight"+stn+"_full_reweight"+stn+"Run-1c.root").c_str());
	TFile *f4 = TFile::Open(("../Plots/MC/dMu/5.4e-18/Fits/edmFits_unblinded_allDecays_WORLD_250MeV_AQ_noVertCorr_accWeight"+stn+"_full_reweight"+stn+"Run-1d.root").c_str());
	...
	TGraphErrors *gr5 = ResetGraph( (TGraphErrors*)f5->Get("MomentumBinnedAnalysis/ParameterScans/AEDM_vs_p_thetaY"), xmin, xmax );
	TGraphErrors *gr6 = ResetGraph( (TGraphErrors*)f6->Get("MomentumBinnedAnalysis/ParameterScans/AEDM_vs_p_thetaY"), xmin, xmax );
	TGraphErrors *gr7 = ResetGraph( (TGraphErrors*)f7->Get("MomentumBinnedAnalysis/ParameterScans/AEDM_vs_p_thetaY"), xmin, xmax );
	TGraphErrors *gr8 = ResetGraph( (TGraphErrors*)f8->Get("MomentumBinnedAnalysis/ParameterScans/AEDM_vs_p_thetaY"), xmin, xmax );
	TGraphErrors *gr9 = ResetGraph( (TGraphErrors*)f9->Get("MomentumBinnedAnalysis/ParameterScans/AEDM_vs_p_thetaY"), xmin, xmax );
	...
	gr5->GetYaxis()->SetRangeUser(0, 0.35);
	...
	TLegend *l1 = new TLegend(.50, .65, .89, .89);
	l1->SetBorderSize(0);
	l1->SetTextSize(24);
	l1->SetTextFont(44);
	...
	l1->AddEntry(gr5, "None");
	l1->AddEntry(gr0, "Acc.");

	l1->AddEntry(gr6, "Run-1a");
	l1->AddEntry(gr1, "Acc. + Run-1a");

	l1->AddEntry(gr7, "Run-1b");
	l1->AddEntry(gr2, "Acc. + Run-1b");

	l1->AddEntry(gr8, "Run-1b");
	l1->AddEntry(gr3, "Acc. + Run-1b");

	l1->AddEntry(gr9, "Run-1b");
	l1->AddEntry(gr4, "Acc. + Run-1d");
	...
	l1->Draw("SAME");
  	...
  	for(int i(0); i<gr5->GetN(); i++) { 
  		...
  		ratio->SetPoint(i, gr5->GetX()[i], gr0->GetY()[i]/gr5->GetY()[i]);
  		ratio->SetPointError(i, 0., gr0->GetEY()[i]);

  		//cout<<"Delta Main = "<<gr0->GetY()[i]/gr5->GetY()[i]<<endl;

  	  	ratio_a->SetPoint(i, gr1->GetX()[i], gr1->GetY()[i]/gr6->GetY()[i]);
  		ratio_a->SetPointError(i, 0., gr6->GetEY()[i]);
  	  	
  	  	cout<<"\nDelta Run-1a = "<<(gr1->GetY()[i]/gr6->GetY()[i]) - (gr0->GetY()[i]/gr5->GetY()[i])<<endl;

  	  	ratio_b->SetPoint(i, gr2->GetX()[i], gr2->GetY()[i]/gr7->GetY()[i]);
  		ratio_b->SetPointError(i, 0., gr7->GetEY()[i]);

  		cout<<"Delta Run-1b = "<<(gr2->GetY()[i]/gr7->GetY()[i]) - (gr0->GetY()[i]/gr5->GetY()[i])<<endl;

  	  	ratio_c->SetPoint(i, gr3->GetX()[i], gr3->GetY()[i]/gr8->GetY()[i]);
  		ratio_c->SetPointError(i, 0., gr8->GetEY()[i]);

		cout<<"Delta Run-1c = "<<(gr3->GetY()[i]/gr8->GetY()[i]) - (gr0->GetY()[i]/gr5->GetY()[i])<<endl;

   	  	ratio_d->SetPoint(i, gr4->GetX()[i], gr4->GetY()[i]/gr9->GetY()[i]);
  		ratio_d->SetPointError(i, 0., gr9->GetEY()[i]);

  		cout<<"Delta Run-1d = "<<(gr4->GetY()[i]/gr9->GetY()[i]) - (gr0->GetY()[i]/gr5->GetY()[i])<<endl;

  	}

  	string foutName = "../Plots/MC/Acceptance/Plots/acceptanceCorrection_250MeV_full_reweight"+stn+".root";

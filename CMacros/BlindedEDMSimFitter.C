...
const double GetPhase(TFile *input, TFile *output, std::string config, std::string dMu) { //, bool noStations) { 
  ...
  DrawModWiggleSim(gr_wiggle_mod, ";t_{g#minus2}^{mod} [#mus];Normalised decays / 149.2 ns", "../Images/MC/"+dname+"/"+dMu+"/MainPlots/fit_mod_wiggle_"+config, recoLabel, double(h1_wiggle_mod->GetEntries()), ymin, ymax);
...
void SimultaneousAnalysis(const double phi, TFile *input, TFile *output, std::string config, std::string dMu, double scaleFactor, bool unblind) { 
  ...
  for(auto& stn : stn_) { 
    ...
    gr_wiggle_mod->SetName((stn+"Wiggle_Modulo_2").c_str());
    gr_wiggle_mod->Write();
    ...
    // New function
    FullEDMFitB(gr_thetaY_mod, 0, OMEGA_A, phi, 0.0375e-6, 0, 1.0, tauGamma, A, 0, G2PERIOD);
    ...
    //DrawFullEDMFitSim(gr_thetaY_vs_t,  ";Decay time [#mus];#LT#theta_{y}#GT [mrad] / 149.2 ns", ("../Images/MC/"+dname+"/"+dMu+"/MainPlots/"+stn+"edmFit_thetaY_vs_t_"+config+"_"+to_string(unblind)).c_str(), "1000 < p [MeV] < 2500", recoLabel, double(nEntries), ymin_thetaY*scaleFactor, ymax_thetaY*scaleFactor, unblind);

    //DrawFullEDMFitSim(gr_thetaY_vs_t,  ";Decay time [#mus];#LT#theta_{y}#GT [mrad] / 149.2 ns", ("../Images/MC/"+dname+"/"+dMu+"/MainPlots/"+stn+"edmFit_thetaY_vs_t_"+config+"_"+to_string(unblind)).c_str(), "1000 < p [MeV] < 2500", recoLabel, double(nEntries), ymin_thetaY*scaleFactor, ymax_thetaY*scaleFactor, unblind);

    TF1 *func = gr_thetaY_vs_t->GetFunction("FullEDMFunc");
    TF1 *func_early = gr_thetaY_vs_t_early->GetFunction("FullEDMFunc");
    ...
    DrawTH1(h1_thetaY_vs_t, "h1_thetaY_vs_t;Frequency [MHz];FFT magnitude / "+to_string(h1_thetaY_vs_t->GetBinWidth(1))+" MHz", "../Images/MC/"+dname+"/"+dMu+"/MainPlots/"+stn+"h1_thetaY_vs_t");
    h1_thetaY_vs_t->SetName((stn+"h1_thetaY_vs_t").c_str());
    h1_thetaY_vs_t->Draw("HIST");
    h1_thetaY_vs_t->Write();

    DrawTH1(FFT_h1_thetaY_vs_t, "FFT_h1_thetaY_vs_t;Frequency [MHz];FFT magnitude / "+to_string(FFT_h1_thetaY_vs_t->GetBinWidth(1))+" MHz", "../Images/MC/"+dname+"/"+dMu+"/MainPlots/"+stn+"FFT_h1_thetaY_vs_t");
    ...
    DrawTH1(h1_res_thetaY_vs_t, "h1_res_thetaY_vs_t;Decay time [#mus];Residual [mrad] / "+to_string(h1_res_thetaY_vs_t->GetBinWidth(1))+" #mus",  "../Images/MC/"+dname+"/"+dMu+"/MainPlots/"+stn+"h1_res_thetaY_vs_t");
    h1_res_thetaY_vs_t->SetName((stn+"h1_res_thetaY_vs_t").c_str());
    h1_res_thetaY_vs_t->Draw("HIST");
    h1_res_thetaY_vs_t->Write();

    DrawTH1(h1_res_thetaY_vs_t_early, "h1_res_thetaY_vs_t_early;Decay time [#mus];Residual [mrad] / "+to_string(h1_res_thetaY_vs_t_early->GetBinWidth(1))+" #mus",  "../Images/MC/"+dname+"/"+dMu+"/MainPlots/"+stn+"h1_res_thetaY_vs_t_early");
    ...
    DrawTH1(FFT_h1_res_thetaY_vs_t, "FFT_h1_res_thetaY_vs_t;Frequency [MHz];FFT magnitude / "+to_string(FFT_h1_res_thetaY_vs_t->GetBinWidth(1))+" MHz",  "../Images/MC/"+dname+"/"+dMu+"/MainPlots/"+stn+"FFT_h1_res_thetaY_vs_t");
    FFT_h1_res_thetaY_vs_t->SetName((stn+"FFT_h1_res_thetaY_vs_t").c_str());
    FFT_h1_res_thetaY_vs_t->Write();

    DrawTH1(FFT_h1_res_thetaY_vs_t_early, "FFT_h1_res_thetaY_vs_t_early;Frequency [MHz];FFT magnitude / "+to_string(FFT_h1_res_thetaY_vs_t_early->GetBinWidth(1))+" MHz",  "../Images/MC/"+dname+"/"+dMu+"/MainPlots/"+stn+"FFT_h1_res_thetaY_vs_t_early");
...
void MomentumBinnedAnalysis(const double phi, TFile *input, TFile *output, std::string config, std::string dMu, double scaleFactor, bool unblind, string dataset, string station, bool reweight) { 
  ...
  for(int i_stn = 0; i_stn < stn_.size(); i_stn++) {
    ...
    for(int i_cut = 0; i_cut < n_cuts; i_cut++) {
      ...
      // DrawModWiggle(gr_wiggle_mod, ";t_{g-2}^{mod} [#mus];#LT#theta_{y}#GT [mrad]", )  
      DrawModWiggleSim(gr_wiggle_mod, ";t_{g#minus2}^{mod} [#mus];Normalised decays / 149.2 ns", ("../Images/MC/"+dname+"/"+dMu+"/MomBinnedAna/"+stn+"fit_mod_wiggle_"+momSlice+"_"+config+"_"+to_string(unblind)).c_str(), recoLabel, double(h1_wiggle_mod->GetEntries()), -0.25, 1.75, to_string(lo)+" < p [MeV] < "+to_string(hi));    
      ...
      // Asymmetry plot (DEFUNCT)
      TH2D *h2_thetaY_mod_clone = (TH2D*)h2_thetaY_mod->Clone("h2_thetaY_mod_clone");
      TH1D *h1_A_mod = GetAsymmetryPlot(h2_thetaY_mod_clone);
      // Figure out blinding for asymmetry ()
      TGraphErrors *gr_A_mod = ConvertToTGraphErrors(h1_A_mod);
      ...
      // Fits  
      FullEDMFit(gr_A_mod, 0, OMEGA_A, phi, 0.0375e-6, 0, 0, G2PERIOD); // Don't worry about this
      FullEDMFitB(gr_thetaY_mod, Ag2, OMEGA_A, phi, Aedm, 0, 1.0, tauGamma, A, 0, G2PERIOD); // "B" includes the wiggle 
...
void Run(string config, string dMu, string dataset, string station, const bool unblind, bool reweight, bool write) {
  ...
  if(dMu=="1700ppm") { // This can problably go, no one seems to care about the longitudinal field 
  ...
  // Jump through some hoops to make sure the output has the correct name 
...
int main(int argc, char *argv[]) {
  ...
  if((dataset == "Run-1a" || dataset == "Run-1b" || dataset == "Run-1c" || dataset == "Run-1d") && ( station == "S12" || station == "S18" || station == "S12S18") ) reweight = true; // eick

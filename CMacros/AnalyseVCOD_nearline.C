...
int main() {
	...
	for( int i_quad = 0; i_quad < N_QHV; i_quad++ ) {
		...
		string input = "../Trees/Data/RadialFieldScan_"+scan+"/merged_noDQC/gm2nearline_hists_run"+runs[i_quad]+".root";
		string output = "../Plots/Data/RadialFieldScan_"+scan+"/VCOD/y-pos_"+runs[i_quad]+".root";

		tuple<vector<double>, vector<double>> yPos = ReadYPos(input, output);//, runs[i_quad]);

		vcods.push_back(yPos);

		TGraphErrors *gr1 = VCOD(yPos);//
		DrawTGraphErrors(gr1, ";#theta [rad];y [mm]", "../Images/Data/VCOD/y_vs_theta_"+to_string(int(QHV[i_quad])));

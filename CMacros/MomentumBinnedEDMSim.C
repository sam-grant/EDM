...
int main() {
	...
	// Read file
	TFile *input = TFile::Open(("../Plots/MC/dMu/5.4e-18/momScan_"+qualString+".root").c_str());
...
//	
		...
		DrawSimpleSinFit(moduloGraph, cut+";t_{g#minus2}^{mod} [#mus];#LT#theta_{y}#GT [mrad]", ("../Images/MC/5.4e-18/MomScan/simpleModuloFit_"+to_string(i_slice)+qualString).c_str(), double(moduloProf->GetEntries()), false);
	...
	DrawTGraphErrors(A_vs_mom, ";Track momentum [MeV];A_{EDM} [mrad]",("../Images/MC/5.4e-18/A_vs_mom"+qualString).c_str());
	DrawTGraphErrors(w_vs_mom, ";Track momentum [MeV];#omega_{a} [MHz]",("../Images/MC/5.4e-18/w_vs_mom"+qualString).c_str());
	DrawTGraphErrors(C_vs_mom, ";Track momentum [MeV];Average vertical offset, c [mrad]",("../Images/MC/5.4e-18/C_vs_mom"+qualString).c_str());

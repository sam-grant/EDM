...
void FiveParFit(TGraphErrors *graph, double par0, double par1, double par2, double par3, double par4, double min, double max) {
...
void BzFit(TGraphErrors *graph, double par0, double par1, double par2, double par3, double par4, double par5, double par6, double min, double max) {
...
//	TF1 *wiggle = graph->GetFunction("FiveParFunc");
	...
	wiggle->SetNpx(1e4);	//wiggle->Draw("SAME");
	//c->SetLogy();
...
int main() {
	...
	bool sanityPlots = true;//false;//true;//false;

	string config = "Bz"; 
	//string qualString = "Q";

	// Read file
	TFile *input = TFile::Open(("../Plots/MC/"+config+"/test.root").c_str());
	...
	TGraphErrors *gr_wiggle_full = ConvertToTGraphErrors(h1_wiggle);
	TGraphErrors *gr_wiggle_mod = ConvertToTGraphErrors(h1_wiggle_mod);
	TGraphErrors *gr_wiggle_mod_shift = ConvertToTGraphErrors(h1_wiggle_mod_shift);
	TGraphErrors *gr_thetaY_mod = ConvertToTGraphErrors(px_thetaY_mod);

	// Draw all base plots
	if(sanityPlots) { 

		DrawTH1(h1_wiggle,"h1_wiggle","../Images/MC/Bz/test/h1_wiggle");
		DrawTH1(h1_wiggle_full,"h1_wiggle","../Images/MC/Bz/test/h1_wiggle_full");
		DrawTH1(h1_wiggle_mod,"h1_wiggle_mod","../Images/MC/Bz/test/h1_wiggle_mod");
		DrawTGraphErrors(gr_wiggle,"gr_wiggle","../Images/MC/Bz/test/gr_wiggle");
		DrawTGraphErrors(gr_wiggle_full,"gr_wiggle_full","../Images/MC/Bz/test/gr_wiggle_full");
		DrawTGraphErrors(gr_wiggle_mod,"gr_wiggle_mod","../Images/MC/Bz/test/gr_wiggle_mod");
		DrawTGraphErrors(gr_wiggle_mod_shift,"gr_wiggle_mod_shift","../Images/MC/Bz/test/gr_wiggle_mod_shift");
		DrawTH2(h2_thetaY_mod,"h2_thetaY_mod","../Images/MC/Bz/test/h2_thetaY_mod");
		DrawTH1(px_thetaY_mod,"px_thetaY_mod","../Images/MC/Bz/test/px_thetaY_mod");
		DrawTGraphErrors(gr_thetaY_mod, "gr_thetaY_mod", "../Images/MC/Bz/test/gr_thetaY_mod");
	...
	// Fit full wiggle
	//FiveParFit(gr_wiggle, 5e3, 64, 1e3, OMEGA_A*1e3, 0);
	//gr_wiggle->GetXaxis()->SetRangeUser(xmin, xmax);
	//DrawFit(gr_wiggle,"fit_wiggle","../Images/MC/Bz/test/fit_wiggle");
...
/*	TF1 *wiggle = new TF1("wiggle","[0] * exp(-x/[1]) * (1 - ([2] *  cos(([3] * x) + [4])))", xmin, xmax);
	...
	// Fit full wiggle
	FiveParFit(gr_wiggle, 1300, 64, 0.35, OMEGA_A*1e3, 0, xmin, xmax);
	...
	DrawFit(gr_wiggle, wiggle,";Time [#mus];Tracks / 149 ns","../Images/MC/Bz/test/fit_wiggle");

	double phi = wiggle->GetParameter(4);

	// Now fit the modulo wiggle

	// No shift 
	FiveParFit(gr_wiggle_mod, 1300, 64, 0.35, OMEGA_A*1e3, 0, 0, G2PERIOD);
	...
	DrawFit(gr_wiggle_mod, modWiggle,";t_{g#minus2}^{mod} [#mus];Tracks / 149 ns","../Images/MC/Bz/test/fit_mod_wiggle");

	// Shift
	FiveParFit(gr_wiggle_mod_shift, 1300, 64, 0.35, OMEGA_A*1e3, 0, 0, G2PERIOD);
	cout<<"G2PERIOD\t"<<G2PERIOD<<endl;
	...
	DrawFit(gr_wiggle_mod_shift, modWiggle_shift,";t_{g#minus2}^{mod} [#mus];Tracks / 149 ns","../Images/MC/Bz/test/fit_mod_wiggle_shift");

	// ======= Fit for A_Bz =====
	// Bz should be 1700 ppm or 0.17 mRad
	BzFit(gr_thetaY_mod, 0.17, OMEGA_A*1e3, phi, 0, OMEGA_A*1e3, phi, 0.5, 0, G2PERIOD);
	...
	DrawFit(gr_thetaY_mod, modWiggle_shift,";t_{g#minus2}^{mod} [#mus];#LT#theta_{y}#GT [mrad]","../Images/MC/Bz/test/fit_Bz");
	...
	//FiveParFit2(h1_wiggle, 1300, 64, 0.35, OMEGA_A*1e3, 0, 0.5*G2PERIOD, xmax-(G2PERIOD/2));
/*
	...
	DrawFit2(h1_wiggle, wiggle2, ";Time [#mus];Tracks / 149 ns","../Images/MC/Bz/test/fit_h1_wiggle");*/

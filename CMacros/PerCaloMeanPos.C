...
int main() {
	...
	for(int i_calo = 1; i_calo<24; i_calo++) {
		...
		for(int i_subrun = 1; i_subrun<400; i_subrun++) { 
		...
		meanY_vs_subruns->GetYaxis()->SetRangeUser(70,80);
		meanYErr_vs_subruns->GetYaxis()->SetRangeUser(0,0.25);
	
		DrawTGraphErrorsDoubleXAxis(meanY_vs_subruns, ";CTAGs;#LTy#GT [mm]", "Sub-runs", "../Images/Data/CaloMeanPos/Split/C"+to_string(i_calo)+"_CaloMeanYPos_vs_CTAG", 0, subruns);
		DrawTGraphErrorsDoubleXAxis(meanYErr_vs_subruns, ";CTAGs;#delta#LTy#GT [mm]", "Sub-runs", "../Images/Data/CaloMeanPos/Split/C"+to_string(i_calo)+"_CaloMeanYPosErr_vs_CTAG", 0, subruns);

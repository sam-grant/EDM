...
void Run(TTree *tree, TFile *output, bool quality) {
   ...
   TH1D *wiggle_mod = new TH1D("Wiggle_Modulo", ";t_{g#minus2}^{mod} [#mus];Tracks", 87, 0, g2Period);
   TH1D *thetaY = new TH1D("ThetaY", ";#theta_{y} [mrad];Tracks", 180, -60, 60);
   TH2D *thetaY_vs_time = new TH2D("ThetaY_vs_Time", ";Decay time [#mus]; #theta_{y} [mrad]", 2700, 0, 2700*0.148936, 180, -60, 60);
   TH2D *thetaY_vs_time_mod = new TH2D("ThetaY_vs_Time_Modulo", ";t_{g#minus2}^{mod} [#mus]; #theta_{y} [mrad]", 87, 0, g2Period, 180, -60, 60);
   ...
   for(int64_t entry = 0; entry < nEntries; entry++) {
      ...
      for ( int i_slice = 0; i_slice < nSlices; i_slice++ ) { 
         ...
         if(p >= double(lo) && p < double(hi)) mom_slices_.at(i_slice)->Fill(g2ModTime, theta_y);

         lo = 400 + i_slice*step; 
         hi = 3000 - i_slice*step;

         if(p >= double(lo) && p < double(hi) && i_slice < nSlices/2) mom_sym_cuts_.at(i_slice)->Fill(g2ModTime, theta_y);

         lo = 0 + i_slice*step;
         hi = pmax;

         if(p >= double(lo) && p < double(hi)) mom_min_scan_.at(i_slice)->Fill(g2ModTime, theta_y);
   ...
   output->mkdir("MomSlices"); output->mkdir("MomSymCuts"); output->mkdir("MomMinScan");

   for ( int i_slice = 0; i_slice < nSlices; i_slice++ ) {
      output->cd("MomSlices"); mom_slices_.at(i_slice)->Write();

...
void Plotter::InitHistos() {
  ...
  for ( int i_slice = 0; i_slice < 15; i_slice++ ) { 
    ...
    plot2D(("ThetaY_vs_Time_Modulo_Slice_"+std::to_string(lo)+"_"+std::to_string(hi)).c_str(), 87, 0, g2Period, 180, -60, 60, "t_{g#minus2}^{mod} [#mus]", "#theta_{y} [mrad]");
...
double VertCorrThetaY(TGraphErrors *c_vs_mom, double theta_y, double p) { 

  double vertOffset = 0; 
  int step = 100;

  // Slice in 100 MeV up to 3000 MeV 
  for ( int i_slice = 0; i_slice < 30; i_slice++ ) { 
    ...
    if(p >= double(lo) && p < double(hi)) {
      double x = (hi + lo) / 2;
      theta_y = theta_y - c_vs_mom->Eval(x);
    }
...
void Plotter::Run() {
  ...
  bool vertCorr = false;//true;//false;//false;//true;

  TFile *input = TFile::Open("../../../../Plots/MC/BzSim/1700ppm/BzSim_unblindedFits_vertOffset_eQ.root");

  if(vertCorr) cout<<"Applying vertex correction with input file "<<input<<endl;
  ...
  while( NextTrEvent() ) {
    ...
    double time = tr->posiInitTime * 1e-3; // ns -> us

    if(quality) { 
//      it(tr->hitVolume)
//      if(tr->trackPValue < 0.05) continue;
      if(time > 70*g2Period) continue;
      if(time < 7*g2Period) continue;
    } 

    int positronID = tr->posiTrackID;

    // if(positronID!=4) continue;
    // cout<<positronID<<endl;

    double px = tr->posiInitPX; 
    double py = tr->posiInitPY;
    double pz = tr->posiInitPZ;
    ...
    if(p > 1800 && p < 3100 && time > 7*g2Period && time < 70*g2Period) {
    ...
    if(vertCorr) {
      TGraphErrors *c_vs_mom = (TGraphErrors*)input->Get("MomentumBinnedAnalysis/ParameterScans/c_vs_p_slice");
      theta_y = VertCorrThetaY(c_vs_mom, theta_y, p);
      delete c_vs_mom;
    }
    ...
    for ( int i_slice = 0; i_slice < 15; i_slice++ ) { 
      ...
      if(p >= double(lo) && p < double(hi)) {
        Fill2D(("ThetaY_vs_Time_Modulo_Slice_"+std::to_string(lo)+"_"+std::to_string(hi)).c_str(), g2ModTime, theta_y);
      }
    ...
    */
    ...
    // Main vertical angle plots (must come after correction)
    if(p > 700 && p < 2400) { 

...
void DrawScanGraph(TGraphErrors *graph, std::string title, std::string fname, int step, bool xLabel) {
  ...
  double pmin = 1000; double pmax = 2500;
...
void DrawModWiggleSim(TGraphErrors *graph, string title, string fname, string reco, double N, string cutStr = "p [MeV] > 1700") {

  TCanvas *c = new TCanvas("c","c",800,600);

  // Getting axis right is usually best done "by hand", but this should get you close
  graph->Draw();
  gPad->Update();
  graph->GetYaxis()->SetRangeUser(gPad->GetUymin() - 0.05, gPad->GetUymax() + 0.1);
...
void DrawFullEDMFitSim(TGraphErrors *graph, std::string title, std::string fname, std::string cutStr, std::string recoLabel, double N, double ymin, double ymax, bool unblind) {
	...
	TF1 *func = graph->GetFunction("FullEDMFuncB");
	func->SetLineWidth(3);
	func->SetLineColor(kRed);
	func->SetNpx(1e5); // (max)

...
double A_MU = 116592059e-11; // This is the combined BNL and FNAL Run-2/3 result. Previously used 116592089e-11 from BNL. 
double GMAGIC = std::sqrt( 1.+1./A_MU );
double TAU = 2.196981; // Muon lifetime in rest frame
double PMAX = 1.01 * M_MU * GMAGIC; 
...
void FitFivePar(TGraphErrors *graph, double par0, double par1, double par2, double par3, double par4, double min, double max, bool fixPhase=false) {
  ...
  // Only for sim.. 
  if(!fixPhase) func->SetParLimits(4, -TMath::Pi()/2, TMath::Pi()/2);
  else func->FixParameter(4, par4);
...
// Numerator only
double FullEDMFuncA(double *x, double *par) { 
  return ( par[0] * TMath::Cos((par[1] * x[0]) + par[2]) ) + ( par[3] * TMath::Sin((par[1] * x[0]) + par[2]) ) + par[4];
}

void FullEDMFitA(TGraphErrors *graph, double par0, double par1, double par2, double par3, double par4, double xmin, double xmax) {
  
  TF1 *func = new TF1("FullEDMFuncA", FullEDMFuncA, xmin, xmax, 5);

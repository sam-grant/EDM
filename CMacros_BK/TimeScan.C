...
int main() {
	...
	// Read file
	TFile *input = TFile::Open(("../Plots/MC/30xBNL/timeScan"+qualString+".root").c_str());
	...
	for ( int i_slice = 0; i_slice < n_slice; i_slice++ ) { 
        ...
        string cut = to_string(int(lo))+"<t<"+to_string(int(hi))+" us";

        cout<<"\n"<<cut<<endl;

        // ============== Sanity plots ==============
       	ThetaY_vs_Time_Modulo_Fine = (TH2D*)input->Get(("ThetaY_vs_Time_Modulo_Fine_"+to_string(i_slice)).c_str());
		DrawTH2(ThetaY_vs_Time_Modulo_Fine, cut, ("../Images/MC/30xBNL/TimeScan/ThetaY_vs_Time_Modulo_2D_"+to_string(i_slice)+qualString).c_str());
	
		TrackMomentum = (TH1D*)input->Get(("TrackMom_"+to_string(i_slice)).c_str()); // ->ProjectionY();
		DrawTH1(TrackMomentum, cut, ("../Images/MC/30xBNL/TimeScan/TrackMom_"+to_string(i_slice)+qualString).c_str());
		
		ThetaY_vs_Time_ProjX = ((TH2D*)input->Get(("ThetaY_vs_Time_"+to_string(i_slice)).c_str()))->ProjectionX();
		DrawTH1(ThetaY_vs_Time_ProjX, cut, ("../Images/MC/30xBNL/TimeScan/ThetaY_vs_Time_ProjX_"+to_string(i_slice)+qualString).c_str());
	
		ThetaY_vs_Time_ProjY = ((TH2D*)input->Get(("ThetaY_vs_Time_"+to_string(i_slice)).c_str()))->ProjectionY();
		DrawTH1(ThetaY_vs_Time_ProjY, cut, ("../Images/MC/30xBNL/TimeScan/ThetaY_vs_Time_ProjY_"+to_string(i_slice)+qualString).c_str());
		
		ThetaY_vs_Time_Modulo_Fine_ProjX = ((TH2D*)input->Get(("ThetaY_vs_Time_Modulo_Fine_"+to_string(i_slice)).c_str()))->ProjectionX();
		DrawTH1(ThetaY_vs_Time_Modulo_Fine_ProjX, cut, ("../Images/MC/30xBNL/TimeScan/ThetaY_vs_Time_Modulo_ProjX_"+to_string(i_slice)+qualString).c_str());
	
		ThetaY_vs_Time_Modulo_Fine_ProjY = ((TH2D*)input->Get(("ThetaY_vs_Time_Modulo_Fine_"+to_string(i_slice)).c_str()))->ProjectionY();
		DrawTH1(ThetaY_vs_Time_Modulo_Fine_ProjY, cut, ("../Images/MC/30xBNL/TimeScan/ThetaY_vs_Time_Modulo_ProjY_"+to_string(i_slice)+qualString).c_str());
		// ==========================================
...
//	
		...
		DrawSimpleSinFit(moduloGraph, cut+";t_{g#minus2}^{mod} [#mus];#LT#theta_{y}#GT [mrad]", ("../Images/MC/30xBNL/TimeScan/simpleModuloFit_"+to_string(i_slice)+qualString).c_str(), double(moduloProf->GetEntries()), true);
		...
		time[i_slice] = hi-(step/2);
		time_err[i_slice] = (step)/2;

	}

	TGraphErrors *A_vs_time = new TGraphErrors(n_slice,time,A,time_err,A_err);
	TGraphErrors *w_vs_time = new TGraphErrors(n_slice,time,w,time_err,w_err);
	TGraphErrors *C_vs_time = new TGraphErrors(n_slice,time,C,time_err,C_err);

	DrawTGraphErrors(A_vs_time, ";Track time [#mus];A_{EDM} [mrad]",("../Images/MC/30xBNL/A_vs_time"+qualString).c_str());

...
void MomentumBinnedAnalysis(TFile *input, TFile *output) { 
	...
	vector<TGraphErrors*> cGraphs_; vector<TGraphErrors*> AEDMGraphs_;
  	...
  	OverlayGraphs(cGraphs_, names_, "", ("../Images/MC/dMuSim/"+config+"/Unblinded/MomBinnedAna/ForJoe/c_vs_p_overlay_"+qual).c_str(), c_ymin, c_ymax);
  	OverlayGraphs(AEDMGraphs_, names_, "", ("../Images/MC/dMuSim/"+config+"/Unblinded/MomBinnedAna/ForJoe/AEDM_vs_p_overlay_"+qual).c_str(), A_ymin, A_ymax);

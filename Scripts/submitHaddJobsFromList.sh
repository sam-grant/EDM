...
read -p "Submitting $nJobs hadd jobs per input directory (1 is OK for all Ntuple configurations except AllDecaysNtuple, where you should use 5). Proceed? <Y/n>" prompt
if [[ $prompt =~ ^[Y]$ ]]; then
    echo ""
else
    printf "\nBye.\n"

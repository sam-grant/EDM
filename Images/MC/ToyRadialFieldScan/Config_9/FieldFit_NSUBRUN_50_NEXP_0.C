void FieldFit_NSUBRUN_50_NEXP_0()
{
//=========Macro generated from canvas: c/c
//=========  (Fri Oct 16 07:54:38 2020) by ROOT version 6.22/02
   TCanvas *c = new TCanvas("c", "c",0,0,800,600);
   c->SetHighLightColor(2);
   c->Range(-45,-0.2205117,45,0.1513556);
   c->SetFillColor(0);
   c->SetBorderMode(0);
   c->SetBorderSize(2);
   c->SetFrameBorderMode(0);
   c->SetFrameBorderMode(0);
   
   Double_t Graph0_fx1017[3] = {
   -30,
   30,
   0};
   Double_t Graph0_fy1017[3] = {
   0.08204483,
   -0.1512007,
   -0.0301199};
   Double_t Graph0_fex1017[3] = {
   0,
   0,
   0};
   Double_t Graph0_fey1017[3] = {
   0.007332894,
   0.007333111,
   0.00733275};
   TGraphErrors *gre = new TGraphErrors(3,Graph0_fx1017,Graph0_fy1017,Graph0_fex1017,Graph0_fey1017);
   gre->SetName("Graph0");
   gre->SetTitle("Sub-runs 50");
   gre->SetFillStyle(1000);
   gre->SetMarkerStyle(20);
   
   TH1F *Graph_Graph01017 = new TH1F("Graph_Graph01017","Sub-runs 50",100,-36,36);
   Graph_Graph01017->SetMinimum(-0.183325);
   Graph_Graph01017->SetMaximum(0.1141689);
   Graph_Graph01017->SetDirectory(0);
   Graph_Graph01017->SetStats(0);

   Int_t ci;      // for color index setting
   TColor *color; // for color definition with alpha
   ci = TColor::GetColor("#000099");
   Graph_Graph01017->SetLineColor(ci);
   Graph_Graph01017->GetXaxis()->SetTitle("Applied B_{r} [ppm]");
   Graph_Graph01017->GetXaxis()->CenterTitle(true);
   Graph_Graph01017->GetXaxis()->SetLabelFont(42);
   Graph_Graph01017->GetXaxis()->SetTitleSize(0.04);
   Graph_Graph01017->GetXaxis()->SetTitleOffset(1.1);
   Graph_Graph01017->GetXaxis()->SetTitleFont(42);
   Graph_Graph01017->GetYaxis()->SetTitle("#LTy#GT / QHV [mm/kV]");
   Graph_Graph01017->GetYaxis()->CenterTitle(true);
   Graph_Graph01017->GetYaxis()->SetNdivisions(4000510);
   Graph_Graph01017->GetYaxis()->SetLabelFont(42);
   Graph_Graph01017->GetYaxis()->SetTitleSize(0.04);
   Graph_Graph01017->GetYaxis()->SetTitleOffset(1.2);
   Graph_Graph01017->GetYaxis()->SetTitleFont(42);
   Graph_Graph01017->GetZaxis()->SetLabelFont(42);
   Graph_Graph01017->GetZaxis()->SetTitleOffset(1);
   Graph_Graph01017->GetZaxis()->SetTitleFont(42);
   gre->SetHistogram(Graph_Graph01017);
   
   
   TF1 *mainFit1018 = new TF1("mainFit","[0]+[1]*x",-36,36, TF1::EAddToList::kNo);
   mainFit1018->SetFillColor(19);
   mainFit1018->SetFillStyle(0);

   ci = TColor::GetColor("#ff0000");
   mainFit1018->SetLineColor(ci);
   mainFit1018->SetLineWidth(2);
   mainFit1018->SetChisquare(0.2464057);
   mainFit1018->SetNDF(1);
   mainFit1018->GetXaxis()->SetLabelFont(42);
   mainFit1018->GetXaxis()->SetTitleOffset(1);
   mainFit1018->GetXaxis()->SetTitleFont(42);
   mainFit1018->GetYaxis()->SetLabelFont(42);
   mainFit1018->GetYaxis()->SetTitleFont(42);
   mainFit1018->SetParameter(0,-0.03309185);
   mainFit1018->SetParError(0,0.004233662);
   mainFit1018->SetParLimits(0,0,0);
   mainFit1018->SetParameter(1,-0.003887424);
   mainFit1018->SetParError(1,0.0001728405);
   mainFit1018->SetParLimits(1,0,0);
   mainFit1018->SetParent(gre);
   gre->GetListOfFunctions()->Add(mainFit1018);
   gre->Draw("ap");
   
   TPaveText *pt = new TPaveText(0.69,0.68,0.89,0.89,"brNDC");
   pt->SetFillColor(0);
   pt->SetTextAlign(33);
   pt->SetTextFont(44);
   pt->SetTextSize(26);
   TText *pt_LaTex = pt->AddText("0.246");
   pt_LaTex = pt->AddText("#minus0.00389#pm0.000173");
   pt_LaTex = pt->AddText("#minus0.0331#pm0.00423");
   pt_LaTex = pt->AddText("#minus8.51#pm 1.15");
   pt->Draw();
   
   pt = new TPaveText(0.3,0.69,0.62,0.88,"brNDC");
   pt->SetFillColor(0);
   pt->SetTextAlign(13);
   pt->SetTextFont(44);
   pt->SetTextSize(26);
   pt_LaTex = pt->AddText("#chi^{2}/ndf");
   pt_LaTex = pt->AddText("Gradient [mm/kV#upointppm]");
   pt_LaTex = pt->AddText("Y-intercept [mm/kV]");
   pt_LaTex = pt->AddText("Background B_{r} [ppm]");
   pt->Draw();
   TLine *line = new TLine(-36,0,-8.51254,0);
   line->SetLineStyle(2);
   line->SetLineWidth(2);
   line->Draw();
   line = new TLine(-8.51254,-0.183325,-8.51254,0);
   line->SetLineStyle(2);
   line->SetLineWidth(2);
   line->Draw();
   
   TF1 *mainFit1019 = new TF1("mainFit","[0]+[1]*x",-36,36, TF1::EAddToList::kNo);
   mainFit1019->SetFillColor(19);
   mainFit1019->SetFillStyle(0);

   ci = TColor::GetColor("#ff0000");
   mainFit1019->SetLineColor(ci);
   mainFit1019->SetLineWidth(2);
   mainFit1019->SetChisquare(0.2464057);
   mainFit1019->SetNDF(1);
   mainFit1019->GetXaxis()->SetLabelFont(42);
   mainFit1019->GetXaxis()->SetTitleOffset(1);
   mainFit1019->GetXaxis()->SetTitleFont(42);
   mainFit1019->GetYaxis()->SetLabelFont(42);
   mainFit1019->GetYaxis()->SetTitleFont(42);
   mainFit1019->SetParameter(0,-0.03309185);
   mainFit1019->SetParError(0,0.004233662);
   mainFit1019->SetParLimits(0,0,0);
   mainFit1019->SetParameter(1,-0.003887424);
   mainFit1019->SetParError(1,0.0001728405);
   mainFit1019->SetParLimits(1,0,0);
   mainFit1019->Draw("same");
   
   pt = new TPaveText(0.3851005,0.94,0.6148995,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(0);
   pt->SetFillColor(0);
   pt->SetFillStyle(0);
   pt->SetTextFont(42);
   pt_LaTex = pt->AddText("Sub-runs 50");
   pt->Draw();
   c->Modified();
   c->cd();
   c->SetSelected(c);
}

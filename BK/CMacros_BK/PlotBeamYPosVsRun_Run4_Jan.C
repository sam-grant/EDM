...
tuple<double, double> ZeroPoint() { 
  ...
  vector<string> runs = csvReader("../txt/CaloBeamPos/CurrentRunNumbers/Run4_Jan.txt");
...
int main() {
 ...
 //string dataset = "Run3NW";
  ...
  vector<string> runs = csvReader("../txt/CaloBeamPos/CurrentRunNumbers/"+dataset+".txt");
 ...
 //DrawTGraphErrors(gr,"tmp","../Images/tmp.png");
  ...
  for( int i_calo = 1; i_calo < 25; i_calo++ ) {
    ...
    for( int i_run = 0; i_run < runs.size(); i_run++ ) {
      ...
      TFile *fin = TFile::Open(input.c_str());
      if(fin==0) { 
        std::cout<<"Run "<<run<<" not opened"<<std::endl;
        continue;
      }
  ...
  DrawManyTGraphErrors2(caloGraphs_meanSub_, caloNames_, ";Run number;Mean subtracted #LTy_{Calo}#GT [mm]", "Day of Jan 2021", "../Images/Data/RadialFieldEstimation/BeamYPosMonitoring/"+dataset+"/y-pos_calos_meanSub_"+runs.at(0)+"-"+runs.at(runs.size()-1), -4.5, 4.5, 1., 31.); 
  //DrawPlotCalo(caloGraphs_meanSub_, caloNames_, ";Run number;Mean subtracted #LTy_{Calo}#GT [mm]", "Day of Jan 2021", "../Images/Data/RadialFieldEstimation/BeamYPosMonitoring/"+dataset+"/y-pos_calos_meanSub_"+runs.at(0)+"-"+runs.at(runs.size()-1), -4.5, 4.5, 1., 31.); 
